  ...
  useEffect(() => {
    const fetchDAU = async () => {
      try {
        ...
        const response = await axios.get(`${BackendUrl}/api/dau`, {
          headers: {
        ...
        if (response.data && response.data.length > 0) {
          ...
          console.log("No DAU data returned from API");
        }
        setIsLoadingData(false);
      } catch (error) {
        console.error("Error fetching DAU:", error);
        setDauError(error.message);
        setIsLoadingData(false);
      }
    };
  ...
  return (
    <div className="main-editor-div">
      ...
      <Card className="w-full">
        ...
        <CardContent>
          {isLoadingData ? (
            <div>Loading DAU data...</div>
          ) : dauError ? (
            <div>Error loading DAU data: {dauError}</div>
          ) : dauData.length === 0 ? (
            <div>No DAU data available</div>
          ) : (
            <div className="h-64">
